    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(this, &MainWindow::showgraph, this, &MainWindow::on_showgraph);

}

MainWindow::~MainWindow()
{
    delete ui, chart, chartWindow;
}
...
void MainWindow::on_showgraph(QVector<double> data) {

    for (int i=0; i<FD; i++) {
        chartGraph->append(i, data[i]);
    }

    chart->removeAllSeries();
    chart->addSeries(chartGraph);
    chart->createDefaultAxes();

    double ymax = chart->axisY()->property("max").toDouble();
    double ymin = chart->axisY()->property("min").toDouble();
    double sp = (ymax - ymin) * 0.2;

    chart->axisY()->setMax(ymax + sp);
    chart->axisY()->setMin(ymin - sp);
...
void MainWindow::init_chart() {
    ...
    if (chart != nullptr) {
        delete chart, chartWindow;
    }

    chart = new QChart();
    chartWindow = new QDialog();
    chartLayout = new QGridLayout(chartWindow);
    chartView = new QChartView(chart, chartWindow);
    chartGraph = new QLineSeries(chartWindow);

    chartLayout->addWidget(chartView);
    chartWindow->setLayout(chartLayout);
    chartWindow->resize(600,400);

    connect(chartWindow, &QDialog::rejected, this, &MainWindow::init_chart);
